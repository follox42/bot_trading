...
def save_trial_backtest_data(
    ...
    """
    Saves all necessary data for a backtest trial with proper organization.
    ...
    Args:
        ...
        position_sizes: Position sizes array
        sl_levels: Stop-loss levels array
        tp_levels: Take-profit levels array
        indicators_array: Array of technical indicators
        indicator_map: Dictionary mapping indicator names to array indices
        account_history: Account metrics history
        long_history: Long position history
        short_history: Short position history
        trades_history: List of trade details
        performance: Performance metrics
        base_dir: Base directory for studies
    """
    # Create directory structure
    study_dir = os.path.join(base_dir, study_name)
    optim_dir = os.path.join(study_dir, "optimizations")
    trial_dir = os.path.join(optim_dir, f"trial_{trial_id}")
    os.makedirs(trial_dir, exist_ok=True)
    
    # 1. Save candlestick and indicator data to CSV
    if indicators_array is not None and indicator_map:
        # Extract indicators into dataframe columns
        indicators_data = {}
        for name, idx in indicator_map.items():
            if idx < len(indicators_array):
                indicators_data[name] = indicators_array[idx]
        
        # Create dataframe with price and indicator data
        candle_data = {
            'price': prices,
            'signal': signals,
        ...
        candle_data.update(indicators_data)
        
        # Save to CSV
        candles_df = pd.DataFrame(candle_data)
        candles_df.to_csv(os.path.join(trial_dir, f"candles.csv"), index=True)
    
    # 2. Save account history data to CSV
    if account_history is not None:
        account_data = {
            "wallet_balance": account_history[:, 0],
            "equity": account_history[:, 1],
            "unrealized_pnl": account_history[:, 2],
            "drawdown": account_history[:, 3]
        }
        
        # Add position data
        if long_history is not None:
            account_data.update({
                "long_active": long_history[:, 0],
                "long_size": long_history[:, 1],
                "long_entry_price": long_history[:, 2],
                "long_leverage": long_history[:, 3],
                "long_margin": long_history[:, 4],
                "long_unrealized_pnl": long_history[:, 5],
                "long_liquidation_price": long_history[:, 6],
                "long_tp_price": long_history[:, 7],
                "long_sl_price": long_history[:, 8]
            })
            
        if short_history is not None:
            account_data.update({
                "short_active": short_history[:, 0],
                "short_size": short_history[:, 1],
                "short_entry_price": short_history[:, 2],
                "short_leverage": short_history[:, 3],
                "short_margin": short_history[:, 4],
                "short_unrealized_pnl": short_history[:, 5],
                "short_liquidation_price": short_history[:, 6],
                "short_tp_price": short_history[:, 7],
                "short_sl_price": short_history[:, 8]
            })
            
        # Save to CSV
        account_df = pd.DataFrame(account_data)
        account_df.to_csv(os.path.join(trial_dir, f"account_history.csv"), index=True)
    
    # 3. Save trades history to a dedicated CSV
    if trades_history:
        trades_df = pd.DataFrame(trades_history)
        trades_df.to_csv(os.path.join(trial_dir, f"trades.csv"), index=False)
    
    # 4. Save performance summary as JSON
    if performance:
        with open(os.path.join(trial_dir, f"summary.json"), 'w') as f:
...
class StrategyOptimizer:
    ...
    def objective(self, trial):
        ...
        et sauvegarde directe des stratégies valables.
        Args:
            trial: Trial Optuna
        Returns:
        ...
        try:
            ...
            try:
                ...
                trades_per_day = total_trades / (len(prices) / 1440)
                ...
                if score > float('-inf') and total_trades >= self.config.min_trades:
                    try:
                        strategy_id = f"trial_{trial.number}_score_{score:.4f}".replace(".", "_")
                        ...
                        for key in ['roi', 'win_rate', 'max_drawdown']:
                            if key in strategy_performance:
                                strategy_performance[f'{key}_pct'] = strategy_performance[key] * 100
                        if hasattr(trial, 'study') and hasattr(trial.study, 'study_name'):
                            current_study = trial.study.study_name
                            if current_study.endswith("_opt"):
                                current_study = current_study[:-4]
                            self.study_manager.save_strategy(
                                study_name=current_study,
                                strategy_rank=trial.number,
                                signal_generator=signal_generator,
                                position_calculator=position_calculator,
                                performance=strategy_performance,
                                strategy_id=strategy_id
                            )
                            study_dir = self.study_manager.get_study_dir(current_study)
                            strategy_dir = os.path.join(study_dir, "strategies", strategy_id)
                            backtest_dir = os.path.join(strategy_dir, "backtests")
                            os.makedirs(backtest_dir, exist_ok=True)
                            backtest_id = f"backtest_{int(time.time())}"
                            
                            # MODIFICATION: Sauvegarde complète dans un seul fichier JSON
                            results_json_path = os.path.join(backtest_dir, f"{backtest_id}_complete.json")
                            try:
                                simulator.save_to_json(results_json_path)
                                self.logger.info(f"Backtest complet sauvegardé dans {results_json_path}")
                                
                                try:
                                    self.study_manager.save_backtest_results(
                                        study_name=current_study,
                                        strategy_id=strategy_id,
                                        ...
                                        results={
                                            ...
                                            "complete_json_path": results_json_path
                                        }
                                    )
                                except Exception as backtest_save_error:
                                    self.logger.warning(f"Erreur lors de l'enregistrement du backtest: {str(backtest_save_error)}")
                            except Exception as json_error:
                                self.logger.warning(f"Erreur lors de la sauvegarde JSON: {str(json_error)}")
                                
                            self.logger.info(f"Stratégie et backtest du trial {trial.number} sauvegardés avec score {score:.4f}")
                        else:
                            self.logger.warning(f"Impossible de déterminer le nom de l'étude pour le trial {trial.number}")
                    except Exception as save_error:
                        self.logger.error(f"Erreur lors de la sauvegarde directe de la stratégie: {str(save_error)}")
                        traceback.print_exc()
