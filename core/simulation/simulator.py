...
# Constantes pour les types d'énumération (pour Numba)
# Trading Mode
TRADING_MODE_ONE_WAY = 0
TRADING_MODE_HEDGE = 1
...
def calculate_liquidation_price(
    ...
    Args:
        ...
        margin_mode_value: Mode de marge (0=Isolé, 1=Cross)
        trading_mode_value: Mode de trading (0=Unidirectionnel, 1=Hedge)
    ...
    if margin_mode_value == MARGIN_MODE_ISOLATED:  # Marge isolée
        ...
        if trading_mode_value == TRADING_MODE_ONE_WAY:  # Unidirectionnel
...
def process_trade_signal(
    ...
    if trading_mode_value == TRADING_MODE_ONE_WAY:
        if signal == 1 and short_pos[0] != 0:
            close_position(short_pos, account, current_price, margin_mode_value, fees, 0.0, stats)
        elif signal == -1 and long_pos[0] != 0:
            close_position(long_pos, account, current_price, margin_mode_value, fees, 0.0, stats)
    ...
    liq_price = calculate_liquidation_price(
        ...
        current_price, margin_mode_value, trading_mode_value, fees, safety_buffer
    )
    target_pos[7] = liq_price
    
    # Mise à jour des métriques
    update_position_metrics(target_pos, current_price)
    update_account_state(account, long_pos, short_pos, margin_mode_value)
...
def check_and_handle_tp_sl(
    ...
    Args:
        ...
        account: État du compte
        current_price: Prix actuel
        margin_mode_value: Mode de marge
    ...
    if hit_tp or hit_sl:
        pnl, is_win = close_position(
            ...
            margin_mode_value, fee_close, slippage, stats
        )
        return True, pnl
...
def simulate_realistic_trading(
    ...
    fee_open: float64,            
    fee_close: float64,           
    margin_mode_value: int64,      # Changé de margin_mode à margin_mode_value (entier)
    trading_mode_value: int64,     # Changé de trading_mode à trading_mode_value (entier)
    tick_size: float64,
    ...
    Args:
        ...
        margin_mode_value: Mode de marge comme entier (0=Isolé, 1=Cross)
        trading_mode_value: Mode de trading comme entier (0=Unidirectionnel, 1=Hedge)
        tick_size: Taille minimale de tick
        min_size: Taille minimum
        max_size: Taille maximum
        safety_buffer: Marge de sécurité pour le prix de liquidation
    ...
    for i in range(n_candles):
        ...
        long_liquidated, long_liq_pnl = check_and_handle_liquidation(
            long_pos, account, current_price, margin_mode_value, fee_close, slippage, stats
        )
        
        short_liquidated, short_liq_pnl = check_and_handle_liquidation(
            short_pos, account, current_price, margin_mode_value, fee_close, slippage, stats
        )
        ...
        if signals[i] != 0 and not (long_liquidated or short_liquidated or long_closed or short_closed):
            success, _ = process_trade_signal(
                ...
                trading_mode_value, margin_mode_value, fee_open,
    ...
    if not is_bankrupt:
        if long_pos[0] != 0:
            pnl, is_win = close_position(long_pos, account, prices[-1], margin_mode_value, fee_close, slippage, stats)
        
        if short_pos[0] != 0:
            pnl, is_win = close_position(short_pos, account, prices[-1], margin_mode_value, fee_close, slippage, stats)
...
class Simulator:
    ...
    def run(
        ...
        # Convertir les enums en valeurs entières pour Numba
        margin_mode_value = int(self.config.margin_mode.value)
        trading_mode_value = int(self.config.trading_mode.value)
        ...
        self.metrics, self.history = simulate_realistic_trading(
            ...
            margin_mode_value=margin_mode_value,  # Utiliser la valeur entière
            trading_mode_value=trading_mode_value,  # Utiliser la valeur entière
    ...
    def save_to_csv(self, base_filepath: Optional[str]) -> List[str]:
        """
        Sauvegarde uniquement les données d'historique en CSV.
        Args:
            base_filepath: Chemin de base pour les fichiers de sortie
        Returns:
            List[str]: Liste des chemins des fichiers sauvegardés
        """
        ...
        # 1. Sauvegarder l'historique des positions et signaux
        if self.trade_history and hasattr(self, 'trade_history'):
            trade_df = pd.DataFrame(self.trade_history)
            trade_path = f"{base_filepath}_trades.csv"
            trade_df.to_csv(trade_path, index=False)
            saved_files.append(trade_path)
        
        # 2. Sauvegarder l'historique du compte et des positions
        account_data = {
        ...
        if hasattr(self, 'indicators_data') and self.indicators_data:
            account_data.update(self.indicators_data)
        
        account_df = pd.DataFrame(account_data)
        account_path = f"{base_filepath}_account.csv"
        account_df.to_csv(account_path, index=True)
        saved_files.append(account_path)
        
        logger.info(f"Historique de simulation sauvegardé dans {', '.join(saved_files)}")
        return saved_files